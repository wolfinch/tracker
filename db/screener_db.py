...
class TrackerDb(object):
    def __init__ (self, trackerCls, tracker_name, read_only=False):
        self.trackerCls = trackerCls
        self.db = init_db(read_only)
        log.info ("init trackerDb : %s "%(tracker_name))
        
        self.table_name = "tracker_%s"%(tracker_name)
...
#             self.mapping = mapper(trackerCls, self.table, non_primary=True)            
    ...
    def db_save_all_data (self, data_l):
        ...
        for s, d in data_l.items():
            c = self.trackerCls(s, d)
            self.db.session.merge (c)
        self.db.session.commit()
    def db_get_data (self):
        log.debug ("retrieving data from db")
        try:
            ResultSet = self.db.session.query(self.mapping).order_by(self.trackerCls.update_time).all()
            log.info ("Retrieved %d tracker data for table: %s"%(len(ResultSet), self.table_name))
            if ResultSet and len(ResultSet):
                ...
                res = self.trackerCls()
                # res_list = [self.trackerCls(c.symbol, c.data) for c in ResultSet]
            #clear cache now
    ...
    def db_get_all_data (self):
        log.debug ("retrieving data from db")
        try:
            ResultSet = self.db.session.query(self.mapping).order_by(self.trackerCls.symbol).all()
            log.info ("Retrieved %d tracker data for table: %s"%(len(ResultSet), self.table_name))
            res_list = {}            
            if (len(ResultSet)):
                for c in ResultSet:
